...
'EN', 'DE' and 'CN'. As with the collection, the default behaviour is to scan the fact
file if there is one, searching for
...
def create_factory(text_file, fact_file, sect_file, fact_type, collection, verbose=False):
    """
...
def  create_elsevier_factory(text_file, fact_file, sect_file, fact_type, verbose=False):
    """
    ...
    if text_tags < 4 :
        return elsevier1.SimpleElsevierSectionFactory(
            text_file, fact_file, sect_file, fact_type)
    else:
